...
##  This plugin is edit by the Others for make it better 

1) Add the offset if set as option --offset;
2) create the multiple folder or class by add --file, eg: --file=One/Two/Three....
3) Deal some bugs, eg: /Config class not found and change app('iseed') to app('\Orangehill\Iseed\Iseed')

 
#### Installation Laravel 5 or later
...
3) Add the service provider by opening a `app/Console/Kernel.php` file, and adding a new item to the `providers` array.
