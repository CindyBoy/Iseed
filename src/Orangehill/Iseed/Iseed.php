...
class Iseed
{
    ...
    protected $databaseSeederPath = '/database/seeds/';
    ...
    public function __construct(Filesystem $filesystem = null, Composer $composer = null)
    {
        ...
        $this->databaseSeederPath  = base_path() . $this->databaseSeederPath;
    }
     ...
     */
    public function generateSeed($table, $database = null, $contentFile = null, $max = 0, $offset, $exclude = null, $prerunEvent = null, $postrunEvent = null, $dumpAuto = true, $indexed = true, $orderBy = null, $direction = 'ASC')
    {
        if (!$database) {
        ...
        // Get the data
        $data = $this->getData($table, $max, $offset, $exclude, $orderBy, $direction);
        ...
        // @Leey defined the namespace
        $namespace ='';
        if($contentFile){
            $len       = strrpos($contentFile, '/', -1);
            $namespace = substr($contentFile, 0, $len);
            $len = $len ? ($len+1) : $len;
            $className = substr($contentFile, $len);
            if($namespace){
                $namespace = 'namespace '.str_replace('/', '\\', $namespace).';';
            }
        }
        $contentFile = $contentFile ?: $className;
        $seedsPath = $this->getPath($contentFile, $seedPath);
        ...
        // Update the DatabaseSeeder.php file, update $className to $contentFile
        $contentFile = $contentFile ? str_replace('/', '\\', $contentFile) : $className;
        return $this->updateDatabaseSeederRunMethod($contentFile) !== false;
    }
     ...
     * @return Array
     */
    public function getData($table, $max, $offset, $exclude = null, $orderBy = null, $direction = 'ASC')
    {
        ...
        if ($max) { 
            // $result = $result->offset(5)->limit($max); 
            $result = $result->offset($offset)->limit($max); 
        }
     ...
     * @return string
     */
    public function populateStub($namespace, $class, $stub, $table, $data, $chunkSize = null, $prerunEvent = null, $postrunEvent = null, $indexed = true)
    {
    ...
    {
        // Check the folder exists or not and then create 
        $path = $path . '/database/seeds/' . $name . '.php';
        $len  = strrpos($path, '/', -5);
    ...
    {
        $databaseSeederPath = $this-> databaseSeederPath. '/DatabaseSeeder.php';
